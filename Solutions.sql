...
-- How many people in each city are estimated to consume coffee, given that 40% of the population does?

SELECT 
	city_name,
	ROUND(
	(population * 0.40)/1000000, 
...
SELECT 
	p.product_name,
	IFNULL(COUNT(s.sale_id),0) as total_orders
FROM products as p
LEFT JOIN
sales as s
...
-- -- Q.5
-- City Population and Pizza Consumers (40%)
-- Provide a list of cities along with their populations and estimated Pizza consumers.
-- return city_name, total current cx, estimated Pizza consumers (40%)

WITH city_table as 
(
	SELECT 
		city_name,
		ROUND((population * 0.40)/1000000, 2) as Pizza_consumers
	FROM city
),
customers_table
AS
...
(
	SELECT 
		city_name, 
		estimated_rent,
		ROUND((population * 0.40)/1000000, 3) as estimated_Pizza_consumer_in_millions
	FROM city
)
SELECT 
	cr.city_name,
